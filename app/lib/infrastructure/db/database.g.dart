...
class NotesFts extends Table
  ...
  @override
  List<GeneratedColumn> get $columns => [title, plainText];
  @override
  String get aliasedName => _alias ?? actualTableName;
  @override
  String get actualTableName => $name;
  ...
  String get moduleAndArgs =>
      'fts5(title, plain_text, content=\'notes\', content_rowid=\'rowid\', tokenize=\'unicode61 remove_diacritics 2\')';
}

class NotesFt extends DataClass implements Insertable<NotesFt> {
  final String title;
  final String plainText;
  const NotesFt({required this.title, required this.plainText});
  @override
  Map<String, Expression> toColumns(bool nullToAbsent) {
    final map = <String, Expression>{};
    map['title'] = Variable<String>(title);
  ...
  NotesFtsCompanion toCompanion(bool nullToAbsent) {
    return NotesFtsCompanion(title: Value(title), plainText: Value(plainText));
  }
  ...
  NotesFt copyWith({String? title, String? plainText}) => NotesFt(
    title: title ?? this.title,
  ...
  @override
  int get hashCode => Object.hash(title, plainText);
  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      (other is NotesFt &&
...
abstract class _$AppDatabase extends GeneratedDatabase {
  ...
  late final Trigger notesFtsInsert = Trigger(
    'CREATE TRIGGER notes_fts_insert AFTER INSERT ON notes BEGIN INSERT INTO notes_fts ("rowid", title, plain_text) VALUES (new."rowid", coalesce(new.title, \'\'), new.document_plain_text);END',
    'notes_fts_insert',
  );
  late final Trigger notesFtsDelete = Trigger(
    'CREATE TRIGGER notes_fts_delete AFTER DELETE ON notes BEGIN INSERT INTO notes_fts (notes_fts, "rowid", title, plain_text) VALUES (\'delete\', old."rowid", coalesce(old.title, \'\'), old.document_plain_text);END',
    'notes_fts_delete',
  );
  late final Trigger notesFtsUpdate = Trigger(
    'CREATE TRIGGER notes_fts_update AFTER UPDATE ON notes BEGIN INSERT INTO notes_fts (notes_fts, "rowid", title, plain_text) VALUES (\'delete\', old."rowid", coalesce(old.title, \'\'), old.document_plain_text);INSERT INTO notes_fts ("rowid", title, plain_text) VALUES (new."rowid", coalesce(new.title, \'\'), new.document_plain_text);END',
    'notes_fts_update',
  );
