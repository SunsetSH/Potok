...
class LocalTitleGenerator {
  ...
  /// годится в заголовок — берём то, что идёт дальше. Снимаются по одному за
  /// проход цикла в [_stripLeadingFillers], поэтому цепочки вроде "ну вот
  /// короче надо купить билеты" разбираются без отдельной записи на каждую
  /// комбинацию — достаточно единичных слов плюс нескольких неразрывных фраз
  /// ("стало быть", "в общем"), которые по отдельности значат другое.
  ///
  /// Многословные фразы идут раньше однословных: иначе "короче" в списке
  /// раньше "короче говоря" забрало бы совпадение первым, и "говоря" осталось
  /// бы висеть в начале заголовка непонятым остатком.
